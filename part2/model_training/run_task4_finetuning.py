...
class FinetuneConfig:
    ...
    batch_size: int = 32
    epochs: int = 200
    lr: float = 1e-4            #I chose a smaller lr for finetuning
    weight_decay: float = 1e-4
    num_workers: int = 0
    device: str = "cuda" if torch.cuda.is_available() else "cpu"
    # fallback if ckpt has no cfg:
    modes: int = 16
    width: int = 64
    use_bn: bool = True
...
def set_bn_eval(m: nn.Module):
    """Freeze BatchNorm running stats (and use running stats) while keeping other layers trainable."""
    if isinstance(m, nn.BatchNorm1d):
        m.eval()

def freeze_bn_running_stats(model: nn.Module):
    """
    Put only BatchNorm layers in eval mode.
    Note: This does NOT freeze BN affine params (weight/bias). They remain trainable unless you disable grads.
    """
...
@torch.no_grad()
def eval_test_t1_unknown(model, test_npy_path: Path, batch_size: int, device: str):
    """
    Evaluate on data_test_unknown_128.npy at t=1.0:
      input  = [u(t0)(x), x, dt=1.0]
      target = u(t=1.0)(x)
    Returns: (avg_relL2, avg_mse)
    """
    ...
    for i in range(0, N, batch_size):
        ...
        mse_sum += mse_fn(pred_b, tgt_b).item()
        n_done += inp_b.size(0)

    avg_rel = rel_sum / n_done
    avg_mse = mse_sum / n_done  
    return avg_rel, avg_mse
...
def main():
    ...
    pre_ckpt_path = Path("results/part2/task3_all2all_time_dependent/best_model.pt")

    results_dir = Path("results/part2/task4_finetune_all2all_unknown")
    ...
    ckpt = torch.load(pre_ckpt_path, map_location=cfg.device)
    ckpt_cfg = ckpt.get("cfg", {}) if isinstance(ckpt, dict) else {}

    modes = int(ckpt_cfg.get("modes", cfg.modes))
    width = int(ckpt_cfg.get("width", cfg.width))
    use_bn = bool(ckpt_cfg.get("use_bn", cfg.use_bn))

    model = TimeDependentFNO1d_All2All(modes=modes, width=width, use_bn=use_bn).to(cfg.device)
            ...
            torch.save(
                {
                    ...
                    "cfg": {
                        **cfg.__dict__,
                        ...
                        "width": width,
                        "use_bn": use_bn,
                        "pretrained_ckpt": str(pre_ckpt_path),
                    },
                },
                best_path,
            )
    ...
    save_curve(
        train_mse_hist, val_mse_hist,
        out_path=results_part2_dir / "loss_curve_task4_finetune_log.png",
        title="Task 4 finetune: Training/Validation MSE loss (log scale)",
        ylabel="MSE",
        yscale="log",
    )
    save_curve(
        train_rel_hist, val_rel_hist,
        out_path=results_part2_dir / "relL2_curve_task4_finetune.png",
        title="Task 4 finetune: Training/Validation relative L2",
        ylabel="Relative L2",
        yscale="linear",
    )
    ...
    test_rel, test_mse = eval_test_t1_unknown(
        model=model,
        test_npy_path=test_path,
        batch_size=cfg.batch_size,
        device=cfg.device,
    )

    print("\n===== TASK 4 FINETUNE: TEST ON UNKNOWN (t=1.0) =====")
    print(f"Average relative L2 error (unknown test, t=1): {test_rel:.6f} ({100*test_rel:.2f}%)")
    print(f"Average MSE (unknown test, t=1):              {test_mse:.6e}")

    test_out_path = results_dir / "test_result_unknown_t1.txt"
    with open(test_out_path, "w") as f:
        f.write(f"AvgRelativeL2_unknown_test_t1: {test_rel:.8f}\n")
        f.write(f"AvgMSE_unknown_test_t1: {test_mse:.8e}\n")

    print(f"\nSaved best finetuned checkpoint to: {best_path}")
    print(f"Saved plots to: {results_part2_dir}")
    print(f"Saved test results to: {test_out_path}")
