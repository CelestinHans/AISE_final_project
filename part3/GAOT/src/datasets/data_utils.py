...
class CustomDataset(Dataset):
    ...
    def __init__(self, c_data: Optional[torch.Tensor], u_data: torch.Tensor, 
                 x_data: torch.Tensor, encoder_graphs: List, decoder_graphs: List,
                latent_queries_split: Optional[List] = None,
                transform: Optional[Callable] = None): 
        """
        ...
        """
        self.latent_queries_split = latent_queries_split
        self.c_data = c_data
        self.u_data = u_data
        self.x_data = x_data
        ...
        if latent_queries_split is not None and len(latent_queries_split) != n_samples:
            raise ValueError("latent_queries_split and u_data must have same number of samples")
    ...
    def __getitem__(self, idx):
        ...
        latent_q = None
        if self.latent_queries_split is not None:
            latent_q = self.latent_queries_split[idx]
...
def collate_variable_batch(batch):
    ...
    # latent_q is either Tensor [N,coord_dim] for each sample (random) or None
    latent_batch = None
    if latent_list[0] is not None:
        latent_batch = torch.stack(latent_list)  # [B, N, coord_dim]
