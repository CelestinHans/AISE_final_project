...
def save_attn_heatmap(A, path, title="Attention"):
...
class StaticTrainer(BaseTrainer):
    ...
    def _init_variable_coords_mode(self, data_splits):
        ...
        all_graphs = self.graph_builder.build_all_graphs(
            ...
            scales=scales,
            build_train=self.setup_config.train,
            tokenization=getattr(self.model_config, "tokenization", "grid"),
            num_latent_tokens=getattr(self.model_config, "num_latent_tokens", 256),
            seed=self.setup_config.seed,
            epoch=None,
            resample_every_epoch=getattr(self.model_config, "resample_every_epoch", False),
            coord_transform=self.data_processor.coord_scaler,

          
            dynamic_radius=getattr(self.model_config, "dynamic_radius", False),
            dynamic_radius_k=getattr(self.model_config, "dynamic_radius_k", 8),
            dynamic_radius_alpha=getattr(self.model_config, "dynamic_radius_alpha", 1.5),
            debug_plot_radii=getattr(self.model_config, "debug_plot_radii", False),
            debug_plot_dir=getattr(self.model_config, "debug_plot_dir", ".results/debug"),
            debug_plot_domain=getattr(self.model_config, "debug_plot_domain", None),
)
    ...
    def _train_step_variable_coords(self, batch):
        ...
        # latent_tokens_coord = self.latent_tokens_coord.to(self.device)
        latent_tokens_coord = latent_batch.to(self.device)  # [B, N, 2]
    ...
    def test(self):
        ...
        print("[DEBUG] coord_used (scaled) min/max:",
            coord_used.min().item(), coord_used.max().item())
        print("[DEBUG] coord_used (scaled) per-dim min/max:",
            coord_used.min(dim=0).values, coord_used.max(dim=0).values)
        ...
        import numpy as np
        print("[DEBUG] coord_plot (physical) per-dim min/max:",
            np.min(coord_plot, axis=0), np.max(coord_plot, axis=0))
        print("[DEBUG] metadata.domain_x:", self.metadata.domain_x)
    ...
    #     x_sample, y_sample, coord_sample, encoder_graph_sample, decoder_graph_sample, latent_batch = batch
    ...
    def _test_step_variable_coords(self, batch):
        """Test step for variable coordinates (vx mode) with optional attention dump."""
        # Strategy II: batch contains latent tokens per sample
        x_sample, y_sample, coord_sample, encoder_graph_sample, decoder_graph_sample, latent_batch = batch
        ...
        # IMPORTANT:
        # - Strategy I (grid): use self.latent_tokens_coord (shared)  -> shape [N_latent, 2]
        # - Strategy II (random): use latent_batch (per sample)       -> shape [B, N_latent, 2]
        if latent_batch is not None and torch.is_tensor(latent_batch) and latent_batch.numel() > 0:
            latent_tokens_coord = latent_batch.to(self.device)
        else:
            latent_tokens_coord = self.latent_tokens_coord.to(self.device)

        # -----------------------------
        # (A) Enable attention capture
        # -----------------------------
        debug_attn = True
        out_dir = ".results/debug/attn"
        prefix = "attn"  # you can set to your PE name if you want, e.g. "coord_mlp"

        def _set_save_attn(model, enabled: bool):
            # model is GAOT
            proc = model.processor  # Transformer
            # encoder layers
            for blk in proc.encoder_layers:
                if hasattr(blk.attn, "save_attn"):
                    blk.attn.save_attn = enabled
                if enabled and hasattr(blk.attn, "last_attn"):
                    blk.attn.last_attn = None
            # middle layer
            if proc.middle_layer is not None:
                if hasattr(proc.middle_layer.attn, "save_attn"):
                    proc.middle_layer.attn.save_attn = enabled
                if enabled and hasattr(proc.middle_layer.attn, "last_attn"):
        ...
        def save_attn_heatmap(attn_2d: torch.Tensor, save_path: str, title: str):
            """
            attn_2d expected shape [L, L] on CPU (float).
            """
            import os
            import matplotlib.pyplot as plt
            os.makedirs(os.path.dirname(save_path), exist_ok=True)

            A = attn_2d.detach().cpu().float()
            plt.figure(figsize=(6, 5))
            plt.imshow(A, aspect="auto")
            plt.colorbar()
            plt.title(title)
        ...
        def _dump_attn_pngs(model, out_dir: str, prefix: str):
            import os
            ...
            for li, blk in enumerate(proc.encoder_layers):
                if hasattr(blk.attn, "last_attn") and blk.attn.last_attn is not None:
                    save_attn_heatmap(
                        blk.attn.last_attn,
                        os.path.join(out_dir, f"{prefix}_enc{li}.png"),
                        title=f"{prefix} | Encoder layer {li}"
                    )

            # middle
            if proc.middle_layer is not None:
                if hasattr(proc.middle_layer.attn, "last_attn") and proc.middle_layer.attn.last_attn is not None:
                    save_attn_heatmap(
                        proc.middle_layer.attn.last_attn,
            ...
            for li, blk in enumerate(proc.decoder_layers):
                if hasattr(blk.attn, "last_attn") and blk.attn.last_attn is not None:
                    save_attn_heatmap(
                        blk.attn.last_attn,
                        os.path.join(out_dir, f"{prefix}_dec{li}.png"),
                        title=f"{prefix} | Decoder layer {li}"
                    )

        if debug_attn:
            _set_save_attn(self.model, True)
        ...
        # -------------------------------------
        # (B) Dump attention and disable capture
        # -------------------------------------
        if debug_attn:
            _dump_attn_pngs(self.model, out_dir=out_dir, prefix=prefix)
            _set_save_attn(self.model, False)

        # Use last sample's coordinates for plotting
        coord_for_plot = coord_sample[-1] if coord_sample.dim() > 2 else coord_sample
