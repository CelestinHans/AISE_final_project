...
class GAOT(nn.Module):
    ...
    def init_processor(self, node_latent_size, config):
        ...
        if self.positional_embedding_name == "coord_mlp":
            pe_dim = patch_volume * self.node_latent_size
            hidden = getattr(config, "coord_pe_hidden", 128)
    ...
    def process(
        ...
        # trategy II: no patching, no grid reshape 
        
        if self.positional_embedding_name == "coord_mlp":
            assert latent_tokens_coord is not None, "latent_tokens_coord is required for coord_mlp PE"


            x = self.patch_linear(rndata)

            # coords shape handling
            coords = latent_tokens_coord
            if coords.dim() == 2:          # [N,2] -> [B,N,2]
                coords = coords.unsqueeze(0).expand(B, -1, -1)
            elif coords.dim() == 3:        # [B,N,2]
                pass
            else:
                raise ValueError(f"latent_tokens_coord must be 2D or 3D, got {coords.shape}")

            # absolute PE via MLP
            x = x + self.coord_mlp(coords)

            # relative bias will use coords (on le passera au Transformer)
            x = self.processor(x, condition=condition, token_coords=coords)
            return x
        ...
        # reshape back (ton code inchangé)
        if self.coord_dim == 2:
            rndata = rndata.view(batch_size, num_patches_H, num_patches_W, P, P, C)
            rndata = rndata.permute(0, 1, 3, 2, 4, 5).contiguous()
            rndata = rndata.view(batch_size, H * W, C)
