...
class DynamicRadiusConfig:
    ...
    debug_plot_path: str = ".results/debug/radii.png"
    debug_plot_split: str = "train"     
    debug_plot_sample_idx: int = 0      
...
class MAGNOConfig:
    ...
    dynamic_radius: DynamicRadiusConfig = field(default_factory=DynamicRadiusConfig)
...
class MAGNOEncoder(nn.Module):
    ...
    def forward(self, 
        ...
        if coord_mode == 'fx':
            # Expect [num_latent, coord_dim]
            if latent_tokens_coord.ndim != 2 or latent_tokens_coord.shape[1] != self.coord_dim:
                raise ValueError(
                    f"Expected latent_tokens_coord shape [num_latent, {self.coord_dim}], got {latent_tokens_coord.shape}"
                )
            latent_tokens_coord_batched = None  # unused
        else:
            # vx mode: accept either shared [N,2] or per-batch [B,N,2]
            if latent_tokens_coord.ndim == 2:
                if latent_tokens_coord.shape[1] != self.coord_dim:
                    raise ValueError(
                        f"Expected latent_tokens_coord shape [num_latent, {self.coord_dim}], got {latent_tokens_coord.shape}"
                    )
                latent_tokens_coord_batched = latent_tokens_coord.unsqueeze(0).expand(batch_size, -1, -1)
            elif latent_tokens_coord.ndim == 3:
                if latent_tokens_coord.shape[0] != batch_size or latent_tokens_coord.shape[2] != self.coord_dim:
                    raise ValueError(
                        f"Expected latent_tokens_coord shape [{batch_size}, num_latent, {self.coord_dim}], got {latent_tokens_coord.shape}"
                    )
                latent_tokens_coord_batched = latent_tokens_coord
            else:
                raise ValueError(f"latent_tokens_coord must be 2D or 3D, got {latent_tokens_coord.shape}")
        ...
        if coord_mode == 'fx':
            ...
            encoded_scales = self._forward_vx_mode(
                x_coord, pndata, latent_tokens_coord_batched, neighbors_per_scale
            )
    ...
    def _forward_vx_mode(self, x_coord, pndata, latent_coord, neighbors_per_batch):
        ...
        for scale_idx in range(len(self.scales)):
            ...
            for b in range(batch_size):
                ...
                latent_b = latent_coord[b]  # [num_latent, coord_dim]

                # Prepare coordinates for kernel
                if self.node_embedding:
                    phys_coord = node_pos_encode(x_b)
                    latent_coord_processed = node_pos_encode(latent_b)
                else:
                    phys_coord = x_b
                    latent_coord_processed = latent_b
...
class MAGNODecoder(nn.Module):
    ...
    def forward(self,
        ...
        # --- Validate / normalize latent_tokens_coord ---
        if coord_mode == 'fx':
            if latent_tokens_coord.ndim != 2 or latent_tokens_coord.shape[1] != self.coord_dim:
                raise ValueError(
                    f"Expected latent_tokens_coord shape [num_latent, {self.coord_dim}], got {latent_tokens_coord.shape}"
                )
            latent_tokens_coord_batched = None
        else:
            if latent_tokens_coord.ndim == 2:
                if latent_tokens_coord.shape[1] != self.coord_dim:
                    raise ValueError(
        ...
        if coord_mode == 'fx':
            ...
            decoded_scales = self._forward_vx_mode(
                latent_tokens_coord_batched, rndata, query_coord, neighbors_per_scale
            )
