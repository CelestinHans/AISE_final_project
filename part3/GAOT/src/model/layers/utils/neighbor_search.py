...
def _torch_cluster_neighbor_search(data: torch.Tensor, queries: torch.Tensor, radius):
    """
    Efficient neighbor search using torch_cluster library.

    Supports:
      - radius: float
      - radius: torch.Tensor of shape [num_queries] (one radius per query)
    """
    if not HAS_TORCH_CLUSTER:
        raise ImportError("torch_cluster library is required for this method")
    ...
    if not isinstance(radius, torch.Tensor):
        row, col = radius_neighbors(data, queries, r=float(radius))
        num_queries = queries.size(0)
        neighbors_index = col.long()
        row_counts = torch.bincount(row, minlength=num_queries)
        neighbors_row_splits = torch.cat(
    ...
    if radius.dim() != 1 or radius.numel() != queries.size(0):
        raise ValueError("If radius is a tensor, it must be 1D and match num_queries.")

    radius = radius.to(device=queries.device, dtype=queries.dtype)
    r_max = float(radius.max().item())


    row, col = radius_neighbors(data, queries, r=r_max)


    diff = data[col] - queries[row]                     # [E, D]
    dist = torch.linalg.norm(diff, dim=1)               # [E]
    keep = dist <= radius[row]                          # [E] compare with local radius
