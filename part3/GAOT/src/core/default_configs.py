...
#     # Strategy II (random sampling)
#     tokenization: str = "grid"            # "grid" | "random"
#     num_latent_tokens: int = 256          # used if tokenization == "random"
#     resample_every_epoch: bool = False    # fixed by default (supports precompute)
#     sampling: str = "from_domain_points"  # simplest: sample among points in D
...
class ModelConfig:
    ...
    # --- Strategy II (Random sampling tokenization) ---
    tokenization: str = "grid"          # "grid" | "random"
    sampling: str = "from_domain_points"
    num_latent_tokens: int = 256
    resample_every_epoch: bool = False

    # --- Dynamic radius (RIGNO-style) ---
    dynamic_radius: bool = False
    dynamic_radius_k: int = 8
    dynamic_radius_alpha: float = 1.5

    # --- Debug plots ---
    debug_plot_radii: bool = False
    debug_plot_dir: str = ".results/debug"
    debug_plot_domain: Optional[List[List[float]]] = None  # e.g. [[-1,-1],[1,1]]
